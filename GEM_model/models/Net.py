...
class Net(nn.Module):
    def __init__(self):
        ...
        ###########Shaona
        cfg.PGM.USEATTEND = 'attentiontransformer'
        FEATURES = ['xyz']

        #cfg.PGM.FEATURES = ['gxyz']
        cfg.PGM.FEATURES = ['lxyz']        
        
        
        #NEIGHBORSNUM = 8
        #FEATURE_EDGE_CHANNEL = 512
        #GCM_model = DGCNN(FEATURES,NEIGHBORSNUM,FEATURE_EDGE_CHANNEL)

        cfg.PGM.NEIGHBORSNUM = 8
        
        cfg.PGM.NEIGHBORSNUM = 6
        

        ########
        ###3D
        ########
        #cfg.PGM.NEIGHBORSNUM = 20        
        ...
        for i in range(self.gnn_layer):
            if i == 0:
                gnn_layer = Siamese_Gconv(1024, cfg.PGM.GNN_FEAT)
            else:
    ...
    def forward(self, P_src, P_tgt):
        ...
        for i in range(self.gnn_layer):
            ...
            if cfg.PGM.USEATTEND == 'attentiontransformer':
                ...
                #print('-----Shaonan scores_src shape:',scores_src.shape)
