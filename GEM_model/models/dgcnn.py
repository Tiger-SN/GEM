...
def knn(x, k):
    ...
    #print('---KNN pairwise_distance shape:',pairwise_distance.shape)
...
def get_graph_feature(data, feature_name, k=20):
    ...
    #print('----neighbor_gxyz shape:',neighbor_gxyz.shape)    
    ...
    #--Feature shape: torch.Size([16, 25, 8, 8])
...
class DGCNN(nn.Module):
    ...
    def forward(self, xyz):
        ...
        #x2.max: torch.Size([16, 64, 8, 1])
        #x3.max: torch.Size([16, 128, 8, 1])
        #x4.max: torch.Size([16, 256, 8, 1])
        #torch.cat shape: torch.Size([16, 512, 8, 1])
        #x_node shape: torch.Size([16, 512, 8, 1])
        
        
        ####################
        #####16 8 36 ->[B Numpoints Dim]
        #####################
        #print('---DGCNN input shape:',xyz.shape)
        xyz = xyz.permute(0, 2, 1).contiguous()  # (B, 3, N)
        #print('DGCNN Input:',xyz.shape)
        batch_size, num_dims, num_points = xyz.size()
    
        
        #print('Input knn size :',xyz.shape)
...
####Shaonan Liu
def get_graph_feature_cross(x, y, k=8):
    # x = x.squeeze()
